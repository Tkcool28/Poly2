...
async def test_second_run_is_a_noop(session):
    ...
    trades = [_trade()]
    async with _make_client({"0xwallet1": trades}) as client:
        assert await ingest_wallet_trades(session, client, wallet) == 1
        assert await ingest_wallet_trades(session, client, wallet) == 0
...
async def test_same_second_different_tx_are_both_kept(session):
    ...
    async with _make_client({"0xwallet1": trades}) as client:
        assert await ingest_wallet_trades(session, client, wallet) == 3
...
async def test_run_ingestion_cycle_sequential_and_isolated(session):
    ...
    assert results == {"0xwallet1": 1, "0xwallet2": 0}
